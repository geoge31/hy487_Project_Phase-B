    ...
    def minimax(self, current_depth, agent_index, gameState): 
        ...
        if (agent_index == 0):  
            for action in gameState.getLegalActions(agent_index):  # For each legal action of pacman
                ...
                temp, score = self.minimax(current_depth , agent_index+1 , next_game_state)
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def alpha_beta(self, current_depth, agent_index, gameState, alpha, beta):
        ...
        if (agent_index == 0):  # If it is max player's (pacman) turn
            for action in gameState.getLegalActions(agent_index):  # For each legal action of pacman
                next_game_state = gameState.generateSuccessor(agent_index, action)
                temp, score = self.alpha_beta(current_depth, agent_index + 1, next_game_state, alpha, beta)
    ...
    def expectimax(self,current_depth,agent_index,gameState):
        ...
        if (agent_index==0):
            for action in gameState.getLegalActions(agent_index):
                ...
                temp,score = self.expectimax(current_depth, agent_index+1, next_gameState)
                if (best_score is None or\
                    score>best_score):
                    best_score = score
                    best_action = action
...
def betterEvaluationFunction(currentGameState):
    ...
    return 1.0/(foodLeft + 1) * foodLeftMultiplier + ghostDistance + \
           1.0/(minFoodist + 1) * foodDistMultiplier + \
           1.0/(capsLeft + 1) * capsLeftMultiplier + extra
